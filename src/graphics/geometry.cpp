...
void Geometry::load(const Renderer& renderer)
{
    ...
    // Load texture via material
    material_.loadTexture(*vkDevice_, device.physicalDevice(), renderer.frame().commandPool(),
...
void Geometry::createUniformBuffers(const Device& device)
{
...
void Geometry::createDescriptorSets(const Pipeline& pipeline)
{
    ...
    for (int i = 0; i < MAX_FRAMES_IN_FLIGHT; ++i)
    {
        ...
        vk::DescriptorImageInfo texInfo(material_.texture().sampler(), material_.texture().view(),
...
Mat4 Geometry::render(const RenderContext& ctx, const Mat4& world)
{
